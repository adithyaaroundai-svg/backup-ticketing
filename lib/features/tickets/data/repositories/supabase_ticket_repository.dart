...
class SupabaseTicketRepository implements TicketRepository {
  ...
  Future<List<Ticket>> getPaginatedTickets({
    ...
    if (statusFilter == 'Open') {
      query = query.inFilter('status', ['New', 'Open', 'In Progress', 'Paused', 'CallBack', 'WontPay', 'CallNotAttended', 'Waiting for Customer', 'BillRaised']);
  ...
  Stream<List<Ticket>> getTickets({String? statusFilter}) {
    return _supabase
      ...
      if (statusFilter == 'Open') {
        return list.where((t) => [
          'New', 'Open', 'In Progress', 'Paused', 'CallBack', 'WontPay', 'CallNotAttended', 'Waiting for Customer', 'BillRaised'
  ...
  Stream<Map<String, int>> getTicketStats() {
    return _supabase
      ...
      for (var map in rows) {
        ...
        } else if (status == 'In Progress' || status == 'Paused' || status == 'CallBack' || status == 'WontPay' || status == 'CallNotAttended') {
