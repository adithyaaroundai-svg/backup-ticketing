...
class ChatToastOverlay extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final dmNewMessage = ref.watch(dmNewMessageEventProvider);
    final customChannelNewMessage = ref.watch(customChannelNewMessageEventProvider);
    ...
    if (dmNewMessage != null && !currentPath.startsWith('/chat/dm/')) {
      activeToast = _ChatToast(
        ...
        channel: 'dm',
        onTap: () {
          ref.read(dmNewMessageEventProvider.notifier).clear();
          context.push('/chat/dm/${dmNewMessage.senderId}');
        },
    ...
    } else if (newMessage != null && !onChatPage) {
      activeToast = _ChatToast(
        ...
        onDismiss: () => ref.read(chatNewMessageEventProvider.notifier).clear(),
      );
    } else if (aroundTallyNewMessage != null && !onAroundTallyPage) {
      activeToast = _ChatToast(
...
class _ChatToastCard extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // Resolve sender display name from agents list
    final agentsAsync = ref.watch(agentsListProvider);
    final List<Map<String, dynamic>> agents =
        (agentsAsync.asData?.value ?? []).cast<Map<String, dynamic>>();

    Map<String, dynamic>? senderMap;
    for (final a in agents) {
      if (a['id'] == message.senderId) {
        senderMap = a;
        break;
      }
    }
    final String senderName = senderMap != null &&
            (senderMap['full_name'] as String?)?.trim().isNotEmpty == true
    ...
    if (channel == 'dm') {
      ...
      channelIcon = LucideIcons.messageCircle;
    ...
    } else if (channel == 'all-aroundtally') {
      channelLabel = 'All-AroundTally';
      channelIcon = LucideIcons.hash;
    } else {
      channelLabel = 'Support';
      channelIcon = LucideIcons.messageSquare;
    }
