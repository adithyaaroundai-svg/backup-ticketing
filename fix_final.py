...
old_positioned = '''                            // Chevron positioned inside the bubble
                            Positioned(
                              bottom: 4,
                              right: isMe ? 4 : null,
                              left: isMe ? null : 4,
...
new_positioned = '''                            // Chevron positioned inside the bubble at top right
                            Positioned(
                              top: 2,
                              right: 2,
                              child: const _HoverableActionMenu(),
  ...
  Widget _buildMoreReactionsButton({
    ...
    return Tooltip(
      ...
      child: InkWell(
        ...
        child: Padding(
          padding: const EdgeInsets.all(6),
          child: Icon(Icons.add_reaction_outlined, size: 24, color: Colors.grey[500]),
        ),
      ),
    );
  }
...
for file in files:
    ...
    # 1. Replace positioned
    if old_positioned in content:
        content = content.replace(old_positioned, new_positioned)
        
    # 2. Replace _HoverableActionMenu class
    ...
    if start_idx != -1:
        ...
        content = content[:start_idx] + menu_replacement + '\n' + content[end_idx:]
